 ...
 */   
class SilaiSchoolCountPcController extends ControllerBase {
	
	public function silaiSchoolCountPcReport(){
		...
		$ngoLists = $reportMasterDataService->getSilaiNgoList($userLocationId);
		//print_r($ngoList);
		//die;
		...
		$output = [];
				...
				$a = 0;
				foreach ($ngoLists as $key => $value) {
					$output[$key]['ngoCode'] = $value['code'];
					$output[$key]['ngoName'] = $value['name'];
					$a = 0;
					...
					foreach ($schoolTypeList as $key2 => $value2) {
						...
						$query->join('node__field_name_of_ngo', 's_ngo', 's_ngo.entity_id = n.nid');
						
						$query->condition('s_status.bundle', ['silai_school']);
						$query->condition('s_type.bundle', ['silai_school']);
						$query->condition('s_location.bundle', ['silai_school']);
						...
						$query->condition('s_location.field_silai_location_target_id', $userLocationId);
						$query->condition('s_ngo.field_name_of_ngo_target_id', $key);
				...
				$output[1111111111]['ngoCode'] = 'Total :-';
				$output[1111111111]['ngoName'] = ' -- ';
				foreach ($schoolTypeList as $key => $value) {
					$headerName = preg_replace('/\s+/', '', $value);
