...
class AddViewsCustomTable extends FormBase {
   ...
   * Class constructor.
   */
  public function __construct(AccountProxy $account, EntityTypeManager $entityManager) {
    $this->step = 1;
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    elseif ($this->step == 2) {
      ...
      $all_custom_tables = view_custom_table_load_tables_info($table_name, $databaseName);
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    if ($this->step == 1) {
      ...
      if ($drupal_connection->schema()->tableExists('custom_table_view_data')) {
        $query = $drupal_connection->select('custom_table_view_data', 'vd')
          ->fields('vd', [
            'id',
          ])
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($opeation == $this->t('Save')) {
      ...
      $drupal_connection = Database::getConnection();
      $result = $drupal_connection->insert('custom_table_view_data')
        ->fields([
          'table_name' => $table_name,
