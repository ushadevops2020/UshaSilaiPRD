...
class studentBulkCertificateIssued extends FormBase {
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'sewing_student_bulk_certificate_issued_update';
  }
  /**
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $cancelRedirectURI = drupal_get_destination();
     $form['description'] = array(
       ...
       HASH_PREFIX      => '<div class="sample-file-download"><a target="_blank" href="/sites/default/files/import_file/student_certificate_update.xls"><i class="fa fa-download"></i> Download Template</a></div>',
     );
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
      ...
      if(!empty($data['error'])) {
        ...
        $operations[] = ['\Drupal\sewing_import\Services\studentBulkCertificateIssuedContent::studentBulkCertificateIssuedContentItem', [$data]];
      }
    }
    if(!empty($operations)) {
      $batch = array(
        ...
        'finished' => '\Drupal\sewing_import\Services\studentBulkCertificateIssuedContent::studentBulkCertificateIssuedContentItemCallback',
      );
      batch_set($batch);
    }
    if(!empty($error)) {
  ...
  public function validateImportData($item, $row) {    
	...
	if(!empty($item[1])){
      $item[1] = $this->getDateFromExcelDate($item[1]);
		...
		$error['error'][$row] = $item[1].' - "Certificate Issued Date" is not valid in row number . '.$row.'.';
	}
/* 	print_r($item);
	die;  */
    if(empty($error)) {
  ...
  public function importLog($error) {
    ...
    $logfile = IMPORT_LOG_PATH."/import-sewing_student-certificate-log_".date("j_M_Y_h_i_s_a").".txt";
