...
class studentBulkAdmissionSumQuery extends FormBase {
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'sewing_student_bulk_asmission_sum_query';
  }
  /**
   * {@inheritdoc}
   */
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
      ...
      if(!empty($data['error'])) {
        ...
        $operations[] = ['\Drupal\sewing_import\Services\studentBulkAdmissionSumQueryContent::studentBulkAdmissionSumQueryContentItem', [$data]];
      }
    }
    if(!empty($operations)) {
      $batch = array(
        ...
        'finished' => '\Drupal\sewing_import\Services\studentBulkAdmissionSumQueryContent::studentBulkAdmissionSumQueryContentItemCallback',
      );
      batch_set($batch);
    }
    if(!empty($error)) {
