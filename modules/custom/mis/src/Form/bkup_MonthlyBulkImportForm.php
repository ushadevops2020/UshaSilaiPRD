...
class MonthlyBulkImportForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['description'] = array(
      ...
      HASH_PREFIX      => '<div class="sample-file-download"><a target="_blank" href="/sites/default/files/import_file/monthly_quarterly_mis_sample_file.xls"><i class="fa fa-download"></i> Download Template</a></div>',
    );
    $form['import_type'] = [
      HASH_TYPE => SELECTFIELD,
      HASH_TITLE => t('MIS Type'),
      HASH_OPTIONS => array('Monthly', 'Quaterly'),
      HASH_REQUIRED => TRUE
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
      ...
      if(!empty($data['error'])) {
        ...
        if(!in_array($data[1], $duplicateFYr[$data[0]])) {
          $finalData = ['\Drupal\mis\Services\addImportContent::addImportContentItem', [$data]];
          $operations[] = $finalData;
        } else {
          $date = date("m/d/Y", $data[0]);
          $error['error'][] = $data[0].'--'.$data[1].' - Data already exists on same fiscal year/Month in row number '.$row.'.';
        }
        $duplicateFYr[$data[0]][] = $data[1];
        $duplicateM[] = $data[1];
      } 
    }
    if(!empty($operations)) {
    ...
    if(!empty($error)) {
      $error = array_values(array_map('current', $error));
  ...
  public function validateImportData($item, $row, $type) {
    ...
    if(empty($monthValid)) {
      $error['error'][] = $fiscalYr.'-'.$monthValue.' - Monthly/Quaterly is not valid in row number . '.$row.'.';
    }
    $query = \Drupal::entityQuery(NODE)
        ->condition(TYPE, 'silai_school')
        ->condition('field_school_code', $item[2]);
    $schoolCode = $query->execute();
    if(empty($schoolCode)) {
        $error['error'][] = $item[2].' - School Code is not valid in row number '.$row.'.';
    } else {
        $nid = array_values($schoolCode);
        ...
        if($record > 0) {
          $error['error'][] = $fiscalYr.'-'.$monthValue.' - Data already exists on same fiscal year in row number . '.$row.'.';
        }
    ...
    if(empty($error)) {
      ...
      $item[] = '';
      $item[] = $data->field_name_of_ngo->target_id;
      $item[] = $data->field_school_type->target_id;
      $item[] = $data->field_silai_business_state->target_id;
      $item[] = $data->field_silai_district->target_id;
      $item[] = $data->field_silai_block->target_id;
      $item[] = $data->field_silai_village->target_id;
      $result = $item;
    } else {
      $result = $error;
    }
